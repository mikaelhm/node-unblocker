...
var re_abs_url = /("|'|=)(http:(\/\/|\\\/\\\/)|https:(\/\/|\\\/\\\/))/ig, // "http, 'http, or =http
    ...
    // maches set document domain in JS files eg. document.domain="twitter.com"
    re_js_document_domain = /(document.domain=(\"|\'))([^\"\']+)(\"|\')/ig;
...
function rewriteUrls(chunk, uri, thisSite) {
    ...
    // fix document.domain in JS files
    chunk = chunk.replace(re_js_document_domain, "$1" + 'localhost' + "$2");
